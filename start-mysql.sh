...
docker run \
  ...
  -v mysql_sanfrancisco:/var/lib/mysql \
  --name mysql-sanfrancisco mysql:5.7

mysql_ip=`docker inspect -f "{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}" mysql-sanfrancisco`;
