...
exports.ReportesVentasService = void 0;
...
let ReportesVentasService = class ReportesVentasService {
    async getServiciosVentas(options, sucursalId) {
        const detVentaQuery = typeorm_1.getRepository(ventasDetalle_entity_1.DetalleVentasEntity)
        ...
        lodash_1.forIn(options.filters, (value, key) => {
            switch (key) {
                case 'fecha':
                    ...
                    detVentaQuery.andWhere('detVenta.createdAt BETWEEN :inicio AND :fin', {
                        inicio,
                        fin,
                    });
                    break;
    ...
    async serviciosVentasXLS(filter, sucursalId) {
        ...
        const detVentaQuery = typeorm_1.getRepository(ventasDetalle_entity_1.DetalleVentasEntity)
            ...
            .where('venta.sucursalId =:sucursalId', { sucursalId: sucursalId });
        lodash_1.forIn(filters, (value, key) => {
            switch (key) {
                case 'fecha':
                    const inicio = moment(value).format('YYYY-MM-DD 00:00:00');
                    const fin = moment(value).format('YYYY-MM-DD 23:59:59');
                    ...
                    break;
                case 'realizaEstudioEn':
                    detVentaQuery.andWhere('servicio.realizaEstudioEn :realizaEstudioEn', {
    ...
    async paginateIngresos(options, sucursalId) {
        const dataQuery = typeorm_1.getRepository(pagos_entity_1.PagoEntity)
    ...
    async getIngresosBySucursalXLS(filter, sucursalId) {
        const filters = JSON.parse(filter);
        const detVentaQuery = typeorm_1.getRepository(pagos_entity_1.PagoEntity)
            .createQueryBuilder('pago')
            .leftJoin('pago.venta', 'venta')
            .leftJoin('venta.sucursal', 'sucursal')
            .leftJoin('venta.paciente', 'paciente')
    ...
    async paginateVentasBySucursal(options, sucursalId) {
    ...
    async getVentasBySucursalXLS(filter, sucursalId) {
        const filters = JSON.parse(filter);
        const detVentaQuery = typeorm_1.getRepository(ventas_entity_1.VentaEntity)
            .createQueryBuilder('venta')
            .leftJoin('venta.sucursal', 'sucursal')
            .leftJoin('venta.paciente', 'paciente')
            .leftJoin('venta.cliente', 'cliente')
    ...
    async reporteVentasPeriodoJson(inicio, fin) {
        const result = await typeorm_1.getConnectionManager()
            .get()
            .query('CALL CalculoIngresosSucursalesPeriodo(?, ?)', [inicio, fin]);
        return result && result[0] ? result[0] : [];
    }
    async reporteVentasPeriodoXls(inicio, fin) {
        ...
        const heading = [
            ...
            [
                {
                    value: `De: ${moment(inicio).format('DD/MM/YYYY')} a: ${moment(fin).format('DD/MM/YYYY')}`,
                    style: styles.header2,
                },
            ],
            [
...
ReportesVentasService = __decorate([
    common_1.Injectable()
], ReportesVentasService);
exports.ReportesVentasService = ReportesVentasService;
//# sourceMappingURL=reportes-ventas.service.js.map